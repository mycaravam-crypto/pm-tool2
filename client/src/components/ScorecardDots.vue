...
const LABELS = { scope: 'Scope', schedule: 'Schedule', cost: 'Cost', quality: 'Quality' };
</script>
...
<template>
  <div class="scorecard-dots flex items-center gap-1" role="group" aria-label="Project scorecard">
    <span
      ...
      :key="key"
      :class="[RAG_COLORS[scorecard[key]], size === 'sm' ? 'w-2 h-2' : 'w-2.5 h-2.5']"
      class="rounded-full inline-block"
      :title="`${LABELS[key]}: ${RAG_LABELS[scorecard[key]]}`"
    />
  </div>
</template>
