...
const allItems = computed(() => {
  let rows = store.events.flatMap(e => [
    ...e.action_items.map(a => ({
      kind: 'action', id: `a${a.id}`, raw: a, text: a.text, date: a.due_date || e.date, event: e, project: e.project,
      person: a.assignee_name,
      statusLabel: a.done ? 'Done' : (isOverdue(a) ? 'Overdue' : 'Open'),
      statusClass: a.done
        ? 'bg-emerald-100 text-emerald-700'
        : (isOverdue(a) ? 'bg-rose-100 text-rose-700' : 'bg-slate-100 text-slate-600')
    })),
    ...e.pain_points.map(p => ({
      kind: 'pain', id: `p${p.id}`, raw: p, text: p.text, date: e.date, event: e, project: e.project,
      person: p.owner_name, statusLabel: p.resolved ? 'Resolved' : p.severity,
      statusClass: p.resolved
        ? 'bg-emerald-100 text-emerald-700'
        : { High: 'bg-rose-100 text-rose-700', Medium: 'bg-amber-100 text-amber-700', Low: 'bg-slate-100 text-slate-600' }[p.severity]
    })),
    ...e.decisions.map(d => ({
      kind: 'decision', id: `d${d.id}`, raw: d, text: d.text, date: e.date, event: e, project: e.project,
      person: d.decided_by_name, statusLabel: 'Decided', statusClass: 'bg-indigo-100 text-indigo-700'
    }))
  ]);
  rows = rows.concat(store.selectedProjects.flatMap(p => [
    ...p.requirements.map(r => ({
      kind: 'requirement', id: `req${r.id}`, raw: r, text: r.text, date: r.created_at.slice(0, 10), event: null, project: p,
      person: null, statusLabel: r.done ? 'Done' : 'Open',
      statusClass: r.done ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-600'
    })),
    ...p.goals.map(g => ({
      kind: 'goal', id: `goal${g.id}`, raw: g, text: g.text, date: g.target_date || g.created_at.slice(0, 10), event: null, project: p,
      person: null, statusLabel: g.achieved ? 'Achieved' : 'Open',
...
const KIND_LABELS = { action: 'Action Item', pain: 'Pain Point', decision: 'Decision', requirement: 'Requirement', goal: 'Goal' };
const KIND_CLASSES = {
  action: 'bg-sky-100 text-sky-700', pain: 'bg-amber-100 text-amber-700', decision: 'bg-slate-200 text-slate-700',
  requirement: 'bg-cyan-100 text-cyan-700', goal: 'bg-fuchsia-100 text-fuchsia-700'
};
...
function clearRequirementFilters() { requirementOpenOnly.value = false; }
...
function clearGoalFilters() { goalOpenOnly.value = false; }
...
const upcomingEvents = computed(() => {
  let rows = store.events.filter(e => ['milestone', 'deadline'].includes(e.type) && e.date >= todayStr && e.date <= in14DaysStr);
