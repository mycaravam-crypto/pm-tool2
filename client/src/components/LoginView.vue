  ...
  <div class="min-h-screen flex items-center justify-center">
    <form class="glass rounded-2xl p-8 w-full max-w-sm space-y-4" @submit.prevent="submit">
      ...
      <div v-if="mode === 'register'">
        ...
        <input v-model="name" type="text" required autofocus class="w-full border border-white/15 rounded-md px-3 py-2 text-sm" />
      </div>
      <div>
        ...
        <input
          ...
          class="w-full border border-white/15 rounded-md px-3 py-2 text-sm"
        />
      </div>
      <div v-if="mode !== 'forgot'">
        <label class="block text-xs font-medium text-slate-400 mb-1">Password</label>
        <input v-model="password" type="password" required minlength="6" class="w-full border border-white/15 rounded-md px-3 py-2 text-sm" />
      </div>
      ...
      <div class="text-center text-xs text-slate-500 space-x-2">
        <button v-if="mode !== 'login'" type="button" class="text-violet-400 hover:underline" @click="switchMode('login')">Back to sign in</button>
        <template v-else>
          <button type="button" class="text-violet-400 hover:underline" @click="switchMode('forgot')">Forgot password?</button>
          <span>&middot;</span>
          <button type="button" class="text-violet-400 hover:underline" @click="switchMode('register')">Sign up</button>
        </template>
      </div>
    </form>
  </div>
