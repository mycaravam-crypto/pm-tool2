...
- **Single-container deployment.** `server/index.js` now serves the built client (`client/dist`) directly with an SPA fallback, so the whole app is one process/one port. A `Dockerfile` (multi-stage, digest-pinned; builds/installs on `node:24-slim`, but the shipped runtime stage is `gcr.io/distroless/nodejs24-debian12:nonroot` — no shell or package manager, running as its built-in non-root `nonroot` user — to keep the production image's vulnerability surface to just glibc/openssl; `server/entrypoint.js` handles first-run demo seeding, since the distroless image has no shell to run a `docker-entrypoint.sh`) and `docker-compose.yml` (named volume for the SQLite data directory, `restart: unless-stopped`) package this up. `GET /healthz` exists for manual/orchestrator health probes. `server/index.js` also handles `SIGTERM`/`SIGINT` for a graceful shutdown (drains requests, closes WS connections and the DB) — see `.env.example` for the container-facing config knobs.
...
**Still open, deliberately:** security headers (helmet/CSP), a health-check-informed monitoring/alerting setup, cleanup of expired sessions/reset tokens, SQLite WAL mode, and CSRF tokens (the existing `sameSite: 'lax'` cookie already blocks most cross-site abuse). Automated coverage: Vitest unit tests for the timeline's scaling/aggregation/lane-assignment algorithms (`client/src/lib/*.test.js`) and Playwright end-to-end tests for its central user flows (`e2e/timeline.spec.js`) on the client; on the server, `node --test` (`server/**/*.test.js`, 173 tests) covers the permission model, scorecard math, recurrence date math, password hashing, migration idempotency, the full CRUD surface of every route (projects/stakeholders/events, including recurring series and CSV import, decisions/action-items/pain-points/requirements/goals/members/notifications/dashboard), the digest and status-report cron logic invoked directly, and the WebSocket layer against a real bound socket (auth-gated handshake, per-member targeted broadcast). Not exhaustive on either side — most client Vue components beyond the timeline (modals, forms, the Stakeholder Directory, Members panel) have no direct coverage, and some server route edge cases inevitably remain untested (see `DEPLOYMENT.md`'s known-risks section).

## 11. Goal & Scope Alignment (implemented)

Goals and Requirements existed (Section 5, tables 12–13) but were disconnected from everything else: no link between a stated outcome (a Goal) and the deliverables meant to produce it (Requirements), and no health signal for Scope at all — the RAG scorecard (Section 6.E) only ever covered Schedule, Cost, and Quality, three of the four legs of the iron triangle from Section 3. This closes that gap, reusing existing patterns rather than adding new subsystems, same discipline as Section 10:
