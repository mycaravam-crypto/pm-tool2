...
router.post('/', (req, res) => {
  ...
  const info = db.prepare('INSERT INTO goals (project_id, text, target_date) VALUES (?, ?, ?)')
...
router.put('/:id', (req, res) => {
  ...
  if (!existing || !canAccessProject(req.member, existing.project_id)) return res.status(404).json({ error: 'goal not found' });
